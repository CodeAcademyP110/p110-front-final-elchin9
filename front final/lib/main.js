...
var index = 0;
var slides = document.querySelectorAll('.photos li');
var slider = $("slider");
var list = slider.find("ul.photos");
...
function TabContentChanger() {
  ...
  var index = $('.tabs li.active').attr("data-index");
...
function MobileTabChanger() {
  $('.tab_content.active').removeClass("active");
  var index = $('.small-tabs li.active').attr("data-index");
  var connectedTab = document.querySelector(".tab_content[data-index=\"".concat(index, "\"]"));
  connectedTab.classList.add("active");
}

$(document).on("click", ".left-menu-arrow", function () {
  var activeLi = document.querySelector('.small-tabs li.active');
...
$(document).on("click", ".hotel-left-arrow", function () {
  var width = $(".slider-carousel ul li").width();
...
function TabHotelChanger() {
  ...
  var index = $('.tabs-hotel li.active').attr("data-index");
  var connectedTab = document.querySelector(".tab-content-hotel[data-index=\"".concat(index, "\"]"));
  connectedTab.classList.add("active");
}
...
function MobileHotelChanger() {
  $('.tab-content-hotel.active').removeClass("active");
  var index = $('.small-hotels-tab li.active').attr("data-index");
  var connectedTab = document.querySelector(".tab-content-hotel[data-index=\"".concat(index, "\"]"));
  connectedTab.classList.add("active");
} // Slider Testimonals


var testSlides = document.querySelectorAll('.slider-list li');
...
$(document).on("click", ".acc_head", function () {
  var accImage = $(this).prev().attr("src");
...
function TabHolidayChanger() {
  ...
  var index = $('.tabs-holiday li.active-holiday').attr("data-index");
  var connectedTab = document.querySelector(".tab-holiday-content[data-index=\"".concat(index, "\"]"));
...
function MobileHolidayChanger() {
  $('.tab-holiday-content.active-holiday').removeClass("active-holiday");
  var index = $('.small-tabs-holiday li.active-holiday').attr("data-index");
  var connectedTab = document.querySelector(".tab-holiday-content[data-index=\"".concat(index, "\"]"));
  connectedTab.classList.add("active-holiday");
} // End Holiday Tabs
...
$(document).on("click", ".style-changer-header", function (e) {
  e.preventDefault();
  var leftChanger = $(".style-changer").css("left");
...
$(document).on("click", ".button-setting", function () {
  var colorHeader = $(".active-skins").parent().css("background-color");
...
var innerSliderIndex = 0;
var innerSlides = $('.inner-photos li');
...
var innerSliderInterval = setInterval(function () {
  innerSlideRight();
}, 2000);
